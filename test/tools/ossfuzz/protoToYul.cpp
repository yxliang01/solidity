...
void ProtoConverter::visit(FunctionCall const& _x)
{
	...
	case FunctionCall::ZERO:
		if (m_functionSigMap.size() > 0)
		{
			yulAssert(m_functions.size() > 0, "Proto fuzzer: No function in scope");
			string funcName = m_functions[_x.func_index() % m_functions.size()];
...
void ProtoConverter::updateFunctionMaps(string var)
{
	unsigned erased = m_functionSigMap.erase(var);
...
void ProtoConverter::addVarsToScope(vector<string>&& _x)
{

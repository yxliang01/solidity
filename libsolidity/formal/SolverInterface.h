...
{
	SortSort(SortPointer _inner):
		Sort(Kind::Sort), inner(std::move(_inner)) {}
	bool operator==(Sort const& _other) const override
	{
		if (!Sort::operator==(_other))
			return false;
...
public:
	...
	static Expression const_array(Expression _sort, Expression _value)
	{
		...
		std::shared_ptr<SortSort> sortSort = std::dynamic_pointer_cast<SortSort>(_sort.sort);
		std::shared_ptr<ArraySort> arraySort = std::dynamic_pointer_cast<ArraySort>(sortSort->inner);
		solAssert(sortSort, "");
		solAssert(arraySort, "");

...
bool BMC::visit(IfStatement const& _node)
{
	...
	if (isRootFunction())
		addVerificationTarget(
			...
			m_context.expression(_node.condition())->currentValue(),
			&_node.condition()
		);
...
void BMC::inlineFunctionCall(FunctionCall const& _funCall)
{
	...
	else
	{
		...
		if (funType->bound())
		{
			...
			funArgs.push_back(m_context.expression(boundFunction->expression())->currentValue());
		}

		for (auto arg: _funCall.arguments())
			funArgs.push_back(m_context.expression(*arg)->currentValue());
		...
		if (returnParams.size() > 1)
		{
			...
			defineExpr(_funCall, m_context.variable(*returnParams.front())->currentValue());
	}
}

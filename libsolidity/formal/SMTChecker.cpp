...
void SMTChecker::analyze(SourceUnit const& _source, shared_ptr<Scanner> const& _scanner)
{
	...
	m_errorReporterReference.clear();
...
bool SMTChecker::visit(ContractDefinition const& _contract)
{
	...
	m_context.saveIntermediateContext(&_contract);
...
bool SMTChecker::visit(FunctionDefinition const& _function)
{
	...
	m_context.saveVariableIndices(&_function);
...
void SMTChecker::endVisit(FunctionDefinition const& _function)
{
	...
	m_context.saveConstraints(portfolio->assertion());
	m_context.saveVariableIndices(&_function);
	m_context.saveIntermediateContext(&_function);
...
void SMTChecker::visitAssert(FunctionCall const& _funCall)
{
	auto portfolio = dynamic_cast<smt::SMTPortfolio*>(m_interface.get());
	m_context.saveConstraints(portfolio->assertion());
	m_context.saveVariableIndices(&_funCall);
	m_context.saveIntermediateContext(&_funCall);

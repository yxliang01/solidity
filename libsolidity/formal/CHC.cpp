...
bool CHC::visit(IfStatement const& _if)
{
	...
	swap(m_functionCallSeen, functionCallSeen);

	SMTEncoder::visit(_if);

	if (m_functionCallSeen)
		eraseKnowledge();

	m_functionCallSeen = functionCallSeen;
...
{
	/// Function calls are handled at the moment,
